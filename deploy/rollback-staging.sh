#!/usr/bin/env bash
#
# rollback-staging.sh — roll MEL Staging back to a previous backup (code + database).
# Hardcoded target. Run ON THE SERVER:
#
#   bash /home/mel/sites/myeventlane_staging/deploy/rollback-staging.sh [BACKUP_DIR|--list]
#
set -euo pipefail
