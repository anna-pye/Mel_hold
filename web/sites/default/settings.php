...
// Docker / environment-based database (see .env.example). Falls back until local
// settings override (settings.local.php / settings.production.php).
if (getenv('DRUPAL_DATABASE_NAME')) {
  $databases['default']['default'] = [
...
/**
 * Configuration sync directory (relative to project root / drupal root).
 */
 ...
 */
$settings['myeventlane_waitlist.token_secret'] = getenv('WAITLIST_TOKEN_SECRET') ?: '';

/**
 * Optional separate pepper for IP hashing (defaults to hash_salt when empty).
 */
$settings['myeventlane_waitlist.ip_pepper'] = getenv('WAITLIST_IP_PEPPER') ?: '';

/**
 * Mail mode: mailhog (local), smtp, or sendmail — used by custom mail helper.
 */
$settings['myeventlane.mail_mode'] = getenv('MEL_MAIL_MODE') ?: 'php';

/**
 * Environment label: local, production, etc. Theme may use for robots/meta.
 */
